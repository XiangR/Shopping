...
<%
	...
	List<Product> products = ProductMgr.getInstance().findProducts(idArray, keyWord, lowNormalPrice, highNormalPrice,
				lowMemberPrice, highMemberPrice, startDate, endDate, pageNo, 2);
...
<%	
	...
	//int productCount = products.size();				//ProductMgr.getInstance().getProducts(products, pageNo, PAGE_SIZE);
...
--></style>
	...
	<%--	
		<form name="search" method="post" action="ComplexSearchResult.jsp">
			...
			<input type="hidden" name="name" value="<%=searchBean.getKeyWord()%>"/>
										...
										<table class="tableborder" cellpadding="2" cellspacing="1">
											<tbody>
												<tr class="smalltxt" bgcolor="#f3f6fa">
													...
													<td>
														&nbsp;
														<a href="ComplexSearchResult.jsp?keyWord=<%=keyWord %>&lowNormalPrice=<%=lowNormalPrice %>&highNormalPrice=<%=highNormalPrice %>
																&lowMemberPrice=<%=lowMemberPrice %>&categoryId=<%=categoryId %>&highMemberPrice=<%=highMemberPrice %>&startDate=<%=strstarDate %>&endDate<%=strendDate %>&pageNo=<%=pageNo+1%>">&gt;</a>&nbsp;
													</td>
													<td>
														&nbsp;
														<a
